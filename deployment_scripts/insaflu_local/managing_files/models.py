...
class Sample(models.Model):
    ...
    def manual_upload(self):
        """sample has been provided fastq path manually, now it is moved to the correct path"""

        if self.file_name_1 is None:
            return

        upload_path_1 = self.get_fastq(TypePath.MEDIA_ROOT, True)

        upload_path_2 = self.get_fastq(TypePath.MEDIA_ROOT, False)

        if self.path_name_1.name is not None and upload_path_1 is not None:
            if not os.path.exists(upload_path_1):
                print("copying file 1")
                shutil.copy(self.path_name_1.name, upload_path_1)
                self.file_name_1 = upload_path_1

        if self.path_name_2.name is not None and upload_path_2 is not None:
            if not os.path.exists(upload_path_2):
                shutil.copy(self.path_name_2.name, upload_path_2)
                self.file_name_2 = upload_path_2
