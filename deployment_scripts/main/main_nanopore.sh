...
if $QCONTROL; then
    ...
    echo -e "Step\tReads" > $LOGD$SUFFIX"_latest.stats"
...
if $DEPLETE && [ ! -f $REFERENCE ]; then
    ...
    minimap2 -x map-ont -t8 $REFERENCE $C1 | samtools view -f 4 | samtools fastq - > $FILT_R1
    
    if [ ! -z $C2 ]; then
        minimap2 -x map-ont -t8 $REFERENCE $C2 | samtools view -f 4 | samtools fastq - > $FILT_R2
    fi
...
if $ASSEMBLE; then
    ...
    if [ $ASSEMBLY_SOFT == "spades" ]; then
        ...
        $BIN"assembly/spades_nano.sh" -a $C1 -d $ASSD -o $ASSD -b $C2 #2>> $LOGD$SUFFIX.assembly.log
        
        elif [ $ASSEMBLY_SOFT == "flye" ]; then
        
        $BIN"assembly/flye_nano.sh" -a $C1 -d $ASSD -o $ASSD -b $C2 #2>> $LOGD$SUFFIX.assembly.log
        
        elif [ $ASSEMBLY_SOFT == "raven" ]; then
        
        $BIN"assembly/raven.sh" -a $C1 -d $ASSD -o $ASSD -b $C2 #2>> $LOGD$SUFFIX.assembly.log
        
        elif [ $ASSEMBLY_SOFT == "miniasm" ]; then
        
        $BIN"assembly/miniasm.sh" -a $C1 -d $ASSD -o $ASSD -b $C2 #2>> $LOGD$SUFFIX.assembly.log
        
    fi
    
    ##################################################
    ### filter assembly
    echo "FILTER ASSEMBLY"
...
if $REMAP; then
    ...
    if ls $PATTERN 1> /dev/null 2>&1; then
        for REF in $CLASSD$SUFFIX*.targets; do
            echo "new taxid"
            REFN=`basename $REF`
            REFN=${REFN%.targets}
            REMAP_REF=`cat $CLASSD$REFN.file`
            mkdir -p $OUTD$SUFFIX"/"$REFN"/"
            echo $REF
            echo $REMAP_REF
            ...
            if [ $REMAP_SOFT == "snippy" ]; then
                ...
                $BIN"remap/minimap2.sh" -a $C1 -r $REMD$REFN.fasta -d $REMD -s "ref" -b $C2 2> $LOGD$SUFFIX".remap.log"
                
            fi
            
            if [ -s $LOGD"assembly.fofn" ] && [ -s $REMD$SUFFIX"/"$REFN"/snps.stats" ]; then
